...
public class MainControllerDM extends MainController {
	private final static Logger log = LogManager.getLogger(MainControllerDM.class.getName());
		...
		for (int rowindex = 0; rowindex < rowCount && !isStartFound; rowindex++) {
			...
			if (controllerRow.getCell(execFlag) != null) {
				if (controllerRow.getCell(execFlag).toString().equals("Y")) {
					...
					// log.info("Execute Flag is N");//Not required 07\07\23
				}
			}

		}
		log.info("In MainController value of pauseExecution:" + pauseExecution);
		...
		try {
			...
			if (!userInteraction.equalsIgnoreCase("FALSE")) {
				...
				response = JOptionPane.showConfirmDialog(webDriver.getFrame(), message,
						"iTAF - Do you wish to Continue...", JOptionPane.YES_NO_OPTION);
				log.info(response);
