...
public class MainControllerDS extends MainController {

	private final static Logger log = LogManager.getLogger(MainControllerDS.class.getName());
	...
	public Reporter ControllerData(String FilePath) throws NullPointerException, Exception {
		...
		for (int rowIndex = startRow; rowIndex < rowCount; rowIndex++) {
			...
			sc_no = controllerRow.getCell(Integer.parseInt(sheetValues.get("SC_NO").toString()));
				...
				if (executeFlag.toString().equalsIgnoreCase("Y")) {
					for (int columnIndex = startCol; columnIndex < colCount && !pauseExecution; columnIndex++) {
						...
						if (controllerTransactionType != null
							...
							log.info("Header : " + mainControllerHeaderNo);
								...
								try {
									...
									if (controllerTransactionType.toString().equalsIgnoreCase("CloseBrowser")) {
										...
										// ExcelUtility.writeReportPAS(webDriver.getReport());
											...
											.equalsIgnoreCase("CloseAndLaunchNewBrowser")) {
										for (int i = 1; i <= 3; i++) {
											try {
												Automation.driver.quit();
												break;
	...
	public void recoveryhandler() {
		try {
			...
			Cell SC_NO_Cell, TC_ID_Cell = null;
			...
			if (SC_NO_Cell.toString().trim().equalsIgnoreCase("pre"))
				updateFlag = "MarkAllAsN";
			else
				updateFlag = "MarkCurrentAsN";

			// Loop from (current failed row + 1) till last row of MainController to mark
			// Applicable row as N
			for (int rowindx = controllerRow.getRowNum() + 1; rowindx < maincontrollersheetrowCount; rowindx++) {
				...
				if (updateFlag.equals("MarkAllAsN")) {
					if (tempRow.getCell(TC_Header_Num).toString().equalsIgnoreCase(TC_ID_Cell.toString())) {
						tempRow.getCell(ExecuteFlag_Header_Num).setCellValue("N");
						allN = allN + 1;
					}
				}
				if (updateFlag.equals("MarkCurrentAsN")) {
					if (tempRow.getCell(TC_Header_Num).toString().equalsIgnoreCase(TC_ID_Cell.toString())
							&& tempRow.getCell(SC_Header_Num).toString().equalsIgnoreCase(SC_NO_Cell.toString())) {
						tempRow.getCell(ExecuteFlag_Header_Num).setCellValue("N");
						selectiveN = selectiveN + 1;
					}
				}
			}
			if (allN > 0)
			...
			if (selectiveN > 0)
				log.info("As other than Pre SC_NO is failed, SC_NO " + SC_NO_Cell.toString() + " against TC "
						+ TC_ID_Cell.toString() + " is marked as N");
	...
	public boolean pauseFun(String message) {
		...
		WebHelperDS.toDate = new Date();
		webDriver.report.setToDate(Config.dtFormat.format(WebHelperDS.toDate));
		webDriver.report.setFromDate(Config.dtFormat.format(WebHelper.frmDate));
