...
public class WebHelperSuite {

	private final static Logger log = LogManager.getLogger(WebHelperSuite.class.getName());
			...
			for (int rowIndex = 1; rowIndex < rowCount && !controller.pauseExecution; rowIndex++) {
				...
				if (executeFlag.toString().equals("Y")) {
					...
					dynamicIndex = WebHelperUtil.getCellData("DynamicIndex", sheetStructure, rowIndex,
							WebHelper.structureHeader);
					//log.info("iTAF:" + logicalName + " " + rowIndex);//Mandar
				...
				case I:
					...
					if (ctrlValue != null) {
						...
						try {
							...
							String errorMessage = e.getMessage();//Print only first line - Mandar
	        				String firstLine = errorMessage.split("\\r?\\n")[0];
				            ...
				            try {
				                ...
				                try {
				                    ...
				                    try {
				                        ...
				                        //log.error(lastE.getMessage(), lastE);//Mandar ---
...
//---------------					
						...
						try {
							...
							try {
								...
								if (iCtr <= 2) {
									...
									//log.error(e1.getMessage());//Mandar	
									String errorMessage = e1.getMessage();
									String firstLine = errorMessage.split("\\r?\\n")[0];
									log.error(firstLine);
								}
			...
			//log.error(e.getMessage(), e);//Mandar
			String errorMessage = e.getMessage();
			String firstLine = errorMessage.split("\\r?\\n")[0];
			log.error(firstLine);
