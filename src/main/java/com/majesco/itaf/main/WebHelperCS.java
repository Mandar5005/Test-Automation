...
public class WebHelperCS {

	private final static Logger log = LogManager.getLogger(WebHelperCS.class.getName());
	...
	// newly Added two Variables for Action Loop
	{
		...
		try {
			...
			String imageType, indexVal, controlName, executeFlag, action, controltype = "", controlID, dynamicIndex,
					newDynamicIndex, rowNo, colNo;// newly, columnName1, CompareText //Sel4
						...
						if ((action.equals("I") && !ctrlValue.isEmpty()) || (action.equals("V") && !ctrlValue.isEmpty())
							...
							if (!controltype.startsWith("Sikuli")) {
								if (!action.equalsIgnoreCase("LOOP") && !controltype.equalsIgnoreCase("Wait")
										...
										&& !controltype.equalsIgnoreCase("Database")
										...
										&& (ctrlValue == null || !ctrlValue.equalsIgnoreCase("IGNORE"))
	...
	public static String doAction(String imageType, String controlType, String controlId, String controlName,
		...
		log.info("In method doAction, ctrlValue -" + ctrlValue);
		...
		try {
			...
			case WebEdit:
				...
				case NC:
					...
					try {
						if (webElement.isEnabled()) {
							...
							Automation.driver.manage().timeouts().pageLoadTimeout(Duration.ofSeconds(5));
				...
				case EnterInput: // Tanuja: 5-19-2022
					if (ctrlValue == null || ctrlValue.trim().equals("")) {
						break;
					}
				...
				case InputDate:
					...
					try {
						...
						if (webElement.isEnabled()) {
							...
							// imageType, ctrlValue);
							WebHelperCS.doAction(null, "ReplaceDefault", null, null, ctrlValue, null, "I", webElement,
			...
			case HandleWSFailure:
				if (!ctrlValue.equalsIgnoreCase("")) {
					try {
						...
						Mouseclick.perform();
						WebHelperCS.doAction(null, "Wait", null, "10", null, null, "NC", webElement, Results,
						...
						WebHelperCS.doAction(null, "Wait", null, "20", null, null, "NC", webElement, Results,
								strucSheet, valSheet, rowIndex, rowcount, rowNo, colNo);
			...
			case WebButton:
				...
				case I:
					...
					if (webElement == null)// Mrinmayee 14/6/2019
					{
						break;
					}
			...
			/*
			 * case ActionClickandEsc : Actions clickandEsc = new
			 * Actions(Automation.driver); Action clickEscAction =
			 * clickandEsc.moveToElement(webElement).click().sendKeys(Keys.ENTER,
			 * Keys.ESCAPE).build(); clickEscAction.perform(); break;
			 */
			...
			/*
			 * case ActionMouseOver : Actions builderMouserOver = new
			 * Actions(Automation.driver); Action mouseOverAction =
			 * builderMouserOver.moveToElement(webElement).build();
			 * mouseOverAction.perform(); break;
			 */
			...
			if (StringUtils.equalsIgnoreCase(Config.applicationPlatform, "policy2015")
				...
				// To handle processing bar in policy 2015 | In other applications it will not
				// create any issue

				// swapnil41444 06/30/2021 isAlertMissing changes: Below code {isAlertMissing}
				// to check Alert is added to avoid
				// handling of processing bar if Browser Alert box is getting displayed.
				// In absence of this code change, script was going into catch block
				// & somehow Alert message box was getting ignored/click to Cancel
				// automatically.

				// swapnil41444 06/30/2021 isAlertMissing changes starts
				boolean isAlertMissing;
				try {
					WebDriverWait w = new WebDriverWait(Automation.driver, Duration.ofSeconds(1));
				...
				// swapnil41444 06/30/2021 isAlertMissing changes ends
	...
	//07/07/2021: commenting code related to TakesScreenshot
	// functionality...
	...
	public static void saveScreenShot(String controlType) {
		if (ITAFWebDriver.isSuiteApplication()) {
			...
			try {
				...
				// Automation.driver).getScreenshotAs(OutputType.FILE);
				screenshot = new AShot().shootingStrategy(ShootingStrategies.viewportPasting(1000))
						.takeScreenshot(Automation.driver);
			} catch (Exception e) {
				log.error(e.getMessage(), e);
			...
			if (controlType.equalsIgnoreCase("Screenshot")) {
				try {
					...
					new_location = subfolder + "\\" + new_fileName + ".jpeg";
	...
	public static boolean waitForSpinnerToDisappear(WebDriver driver, final WebElement spinnerName) {
		...
		FluentWait<WebDriver> fWait = new FluentWait<>(driver).withTimeout(Duration.ofSeconds(180))
				.pollingEvery(Duration.ofSeconds(1)).ignoring(NoSuchElementException.class);
